...
export default function FavoritesBar({ favorites }: FavoritesBarProps) {
  return (
    <section
    ...
    >
      ...
      <div className="mt-4 flex gap-3 flex-wrap">
        {favorites.map((city, index) => {
          return (
            <div
              className="flex border-1 border-gray-200 bg-[#f9f9f9] px-3 rounded-full hover:scale-102 dark:bg-[#1a2134] dark:border-gray-800 dark:text-gray-400 dark:hover:text-gray-300"
              key={index}
            >
              <X className="w-3 hover:text-red-600" />
              <div className="text-sm mx-1">{city}</div>
            </div>
          );
        })}
      </div>
